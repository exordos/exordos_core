...
Команда читает конфигурационный файл проекта — `exordos.yaml` в корне проекта, `exordos/exordos.yaml` или `genesis/genesis.yaml` (мастер `exordos init` создаёт именно эту структуру). Это основной конфигурационный файл, который превращает ваше приложение в элемент платформы. Он содержит секцию `build`, описывающую:
