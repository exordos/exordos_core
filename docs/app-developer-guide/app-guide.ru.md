...
Настройте существующий проект как элемент Exordos Core. Команда `exordos init` запускает интерактивный мастер, который генерирует все необходимые конфигурационные файлы — включая манифесты и файл `genesis/genesis.yaml`, который описывает процедуры сборки, публикации и развёртывания.
...
- **Публичную установку Exordos** — размещённую на [exordos.com](https://exordos.com). Не требуется настройка; просто создайте аккаунт и начните использовать платформу.
- **Приватную установку Exordos** — ваш собственный self-hosted экземпляр. Этот документ описывает, как его настроить: [Локальное развёртывание](../usage/local_deployment.md).
...
- [Написание манифеста с нуля](../em/manifest.md) — изучите полную спецификацию манифеста и создайте его вручную без помощи мастера `exordos init`.
- [Настройка приватной установки платформы](../usage/local_deployment.md) — разверните собственный экземпляр Exordos Core для разработки и тестирования ваших элементов end-to-end без подключения к удалённому окружению.
