...
Set up an existing project as a Exordos Core element. The `exordos init` command launches an interactive wizard that generates all required configuration files — including manifests, and a `genesis/genesis.yaml` file that describes the build, publish, and deploy procedures.
