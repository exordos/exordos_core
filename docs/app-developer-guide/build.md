...
The command reads the project configuration file — `exordos.yaml` in the project root, `exordos/exordos.yaml`, or `genesis/genesis.yaml` (the layout generated by `exordos init`). This is the main configuration file that transforms your application into a platform element. This file contains the `build` section describing:
...
└── exordos-elements/       # Element repository layout used for push
    └── <element-name>/
        └── <version>/
