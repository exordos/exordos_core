...
Version constraints are checked at install time. If a required element is not installed or its version
falls outside the specified range, the installation fails with a validation error.
...
| `kind` | string | Import kind. Currently only `"resource"` is supported. May be omitted. |
...
An element cannot be uninstalled while other elements import its exports. The platform checks for
dependent elements before removing an element and its resources.
...
1. **Install** — `Manifest.install()` parses the YAML, applies imports, applies exports, then creates
   resource records. Each resource starts in `NEW` status.
...
- **Uninstall** — an element can only be removed if no other elements import its exports. The platform
