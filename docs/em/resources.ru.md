...
- Значение доступно только на запись: его можно задать и заменить, но оно никогда не возвращается при
  чтении. `default_value` скрыт точно так же. Из манифеста обращайтесь к значению через параметр ссылки
  `:value`, который читает значение, доставленное на data plane, либо ссылайтесь на сам секрет через
  `:uuid`.
