...
| `auto_renew` | boolean | Продлевать токен до истечения срока (по умолчанию: `true`). |
| `audience` | string | Claim `aud`. По умолчанию — `client_id` клиента. |
| `issuer` | string | Claim `iss`. По умолчанию — URL клиента. |
...
- Подписанный токен доступен через параметр ссылки `:access_token`, например
  `f"{$core.iam.tokens.$exporter_token:access_token}"` в теле конфигурации. User API его никогда не
  возвращает.
- Когда проходит половина времени жизни, платформа продлевает токен на `expiration_delta`. Токен
...
- Выбирайте время жизни достаточно большим, чтобы продления были редкими: каждое переписывает всё, что
  использует токен. При `auto_renew: false` токен истекает один раз и больше не продлевается.
