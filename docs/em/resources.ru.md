...
| [`$core.iam.tokens`](#coreiamtokens) | IAM-токены доступа с автоматическим продлением |
...
| `description` | string | Человекочитаемое описание. |
| `value` | string | Непрозрачное значение. Необязательно, до 10240 символов. |
| `default_value` | string | Значение, используемое, пока не задано `value`. Необязательно, до 10240 символов. |
...
- User API возвращает `value` и `default_value` всем, кому разрешено читать секрет. Из манифеста
  обращайтесь к значению через параметр ссылки `:value`, который читает значение, доставленное на data
  plane, либо ссылайтесь на сам секрет через `:uuid`.
- `default_value` манифест задаёт тогда, когда знает работающее значение; оператор переопределяет его
  через API. `value` имеет приоритет, а если его очистить, снова используется значение по умолчанию.
- Если не задано ни `value`, ни `default_value`, доставлять нечего: секрет остаётся в статусе `NEW`, а
  элемент, который рендерит из него `:value`, ждёт, пока значение не появится.
...
Токен доступа пользователя, подписанный IAM-клиентом. Платформа продлевает его до истечения срока,
поэтому элемент может выдать сервису учётные данные, которые продолжают работать без входа в систему.

### Поля

| Поле | Тип | Описание |
|---|---|---|
| `user` | uuid | Пользователь, от имени которого действует токен. **Обязательно.** |
| `iam_client` | uuid | IAM-клиент, подписывающий токен. **Обязательно.** Клиент по умолчанию — `00000000-0000-0000-0000-000000000000`, либо ссылка на клиент из манифеста: `$core.iam.clients.$my_client:uuid`. |
| `scope` | string | Область действия токена, например `project:<uuid>`, чтобы ограничить токен проектом. |
| `expiration_delta` | integer | Время жизни в секундах, не меньше 60 (по умолчанию: 3600). |
| `auto_renew` | boolean | Поддерживать токен после истечения срока (по умолчанию: `true`). При `false` токен заканчивается вместе со сроком. |
| `audience` | string | Claim `aud`. Если не задан — `client_id` текущего клиента. |
| `issuer` | string | Claim `iss`. Если не задан — URL текущего клиента. |
...
- Подписанный токен доступен через параметр ссылки `:access_token`, например
  `f"{$core.iam.tokens.$exporter_token:access_token}"` в теле конфигурации. User API отдаёт подписанный
  токен один раз — в ответе на создание — и больше никогда: ни чтение, ни список его не содержат,
  поэтому потерянный токен заменяют новым.
- `POST /v1/iam/tokens/<uuid>/actions/regenerate/invoke` подписывает новый токен взамен выданного
  раньше и отдаёт его один раз — так же, как создание. Токен сохраняет свой uuid, поэтому всё, что
  использует `:access_token`, получает новое значение, а срок жизни начинается заново с этого момента.
  Предыдущий токен перестаёт работать сразу: в отличие от продления, перевыпуск его отвергает.
  Перевыпускайте утёкший токен; для этого нужен `iam.token.update`.
- Когда проходит половина времени жизни, платформа продлевает токен с `auto_renew` на
  `expiration_delta`. Токен
  доступа меняется, и все ресурсы, использующие `:access_token`, обновляются. Предыдущий токен
  действует до зашитого в него срока, поэтому у потребителей есть вторая половина времени жизни, чтобы
  получить новый.
- Выбирайте время жизни достаточно большим, чтобы продления были редкими: каждое переписывает всё, что
  использует токен.
- Изменение `scope` переносит токен в новый проект, а изменение `expiration_delta` начинает новый срок
  жизни с этого момента. Продлевается токен или нет, решается при выпуске: `auto_renew` потом нельзя
  изменить, поэтому срочный токен не станет бессрочным.
- Токен получает разрешения своего пользователя в проекте из `scope`, поэтому назначьте пользователю
  роль в этом проекте.
- Через `/v1/iam/tokens/` аккаунт управляет своими токенами: `iam.token.create`, `read`, `update` и
  `delete` привязаны к роли `owner` и действуют только на токены того, кто обращается. Чужие токены
  требуют `iam.token.read_all`, а выпуск токена за другого пользователя — `iam.token.create_all`;
  и то, и другое есть только у admin. Манифест объявляет токен для любого пользователя, потому что
  идёт мимо API.
