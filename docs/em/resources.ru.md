...
Эта страница документирует каждый тип ресурса, доступный в манифестах Exordos Core. Каждый тип — это
ключ верхнего уровня в секции `resources` манифеста. Структуру манифеста и концепцию ресурсов см. в
[Руководстве по манифестам](manifest.md).
...
| [`$core.secret.certificates`](#coresecretcertificates) | TLS-сертификаты (ACME, вручную) |
...
| `placement_policies` | array | Необязательный список UUID гипервизоров для пиннинга. |
...
**`template`** — шаблон Jinja2, рендерится с переменными платформы:
...
- Поле `on_change` может указывать сервис для перезапуска при изменении конфига:
  `{"kind": "restart_service", "service": "<service-uuid>"}`.
...
setter:
  kind: profile
  fallback_strategy: ignore    # или "default"
  profiles:
...
setter:
  kind: selector
  selector_strategy: latest    # или "manual"
```
...
- `fallback_strategy: ignore` означает, что переменная не имеет значения, если ни один профиль не
  совпал. Используйте `fallback_strategy: default` с полем `default` для предоставления значения по
  умолчанию.
...
- Когда несколько значений привязаны к одной переменной, `selector_strategy: latest` выбирает последнее
  созданное; `selector_strategy: manual` требует явного выбора администратором.
...
TLS-сертификат, управляемый платформой. Поддерживает автоматическое получение через ACME (Let's Encrypt
с DNS-01 challenge) или ручную загрузку сертификата.
...
| `overcome_threshold` | boolean | Если true, обновлять даже если порог ещё не достигнут. |
...
| `value` | string | Значение пароля (обязательно для `MANUAL`, игнорируется для авто-методов). |
...
- Авто-генерируемые пароли создаются один раз при установке и хранятся в зашифрованном виде. Значение
  доступно через параметр ссылки `:value`.
...
- UUID ролей предопределены платформой. См. страницу [IAM Permissions](../iam/permissions_overview.md)
  для списка встроенных ролей.
...
- [Спецификация OpenAPI](https://github.com/exordos/exordos_core/blob/master/docs/openapi/openapi_user.yaml) — детали API на уровне полей
