...
disk_spec:
  ...
  speed: "HOT"      # необязательно: COLD | WARM (по умолчанию) | HOT
  ephemeral: true   # необязательно: по умолчанию false
```
...
Каждый диск (корневой или дополнительный) поддерживает два необязательных свойства для выбора пула хранения:

- `speed` — `COLD`, `WARM` (по умолчанию) или `HOT`.
- `ephemeral` — `true` или `false` (по умолчанию `false`).

Шедулер предпочитает пул с точным совпадением `speed` и достаточным свободным местом; если точного
совпадения нет или подходящий пул заполнен, размещение переходит на любой пул со свободным местом, а не
завершается ошибкой. `ephemeral` — это жёсткое требование, а не предпочтение: диск размещается только на
пуле с тем же значением `ephemeral`, поскольку отклонение от этого правила могло бы разместить durable-диск
на хранилище, которое очищается при перезагрузке хоста.
