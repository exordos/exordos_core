...
This page documents every resource kind available in Exordos Core manifests. Each kind is a top-level key
under the `resources` section of a manifest. For the manifest structure and the resource concept, see the
...
| [`$core.secret.certificates`](#coresecretcertificates) | TLS certificates (ACME, manual) |
...
| `placement_policies` | array | Optional list of hypervisor UUIDs for pinning. |
...
**`template`** — a Jinja2 template rendered with platform variables:
...
- The `on_change` field can specify a service to restart when the config changes:
...
setter:
  kind: profile
  fallback_strategy: ignore    # or "default"
  profiles:
...
setter:
  kind: selector
  selector_strategy: latest    # or "manual"
```
...
A TLS certificate managed by the platform. Supports automatic provisioning via ACME (Let's Encrypt with
DNS-01 challenge) or manual certificate upload.
...
| `overcome_threshold` | boolean | If true, renew even if not yet within threshold. |
...
| `value` | string | The password value (required for `MANUAL`, ignored for auto methods). |
...
- Auto-generated passwords are created once at install time and stored encrypted. The value is available
  via `:value` link parameter.
...
- Role UUIDs are predefined by the platform. See the [IAM Permissions](../iam/permissions_overview.md) page
  for the list of built-in roles.
...
- [OpenAPI specification](https://github.com/exordos/exordos_core/blob/master/docs/openapi/openapi_user.yaml) — field-level API details
