...
Ответ содержит по одной записи на каждую привязку роли. Проект описывают два
разных поля:

* `project_id` — область действия самой роли. Заполнено только для ролей,
  существующих внутри одного проекта; `null` для глобальных ролей, таких как
  `owner` или `admin`.
* `project` — проект, в котором роль выдана данной привязкой. `null`, если
  привязка создана без проекта.
