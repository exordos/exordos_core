...
`ec-mcp-api` обслуживает
[Model Context Protocol](https://modelcontextprotocol.io/), поэтому ИИ-агент
может управлять платформой с теми же учетными данными и правами, что и
обычный клиент API. На инсталляции адрес —
`https://<your-domain>/api/core/v1/mcp`, сам сервис отвечает на
`http://127.0.0.1:11014/v1/mcp`.

Это отдельный сервис рядом с User API, а не его часть. Вызов инструмента
становится обычным HTTP-запросом к User API с учетными данными вызывающего,
поэтому что именно разрешено, решает User API — так же, как для обычного
клиента. Сервис не хранит учетных данных и не обращается к базе: без токена
вызывающего он не может ничего.
...
применяются так же, как к прямым вызовам API. `X-OTP` тоже передается, вместе
с заголовками, которые читают верификаторы правил безопасности
(`X-Firebase-AppCheck`, `X-Goog-Firebase-AppCheck` и `X-Captcha`). Больше
ничего не передается.

## Настройка

`user_api_url` — базовый адрес, по которому сервис обращается к User API.
Указывайте адрес, которым пользуются сами клиенты: User API строит абсолютные
URL по тому хосту, на котором его спросили, поэтому внутренний адрес здесь
попадет во все возвращаемые ссылки.
