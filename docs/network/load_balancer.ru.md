...
- Действия
    ...
    - `local_dir` — отдавать статические файлы из локальной директории на LB;
      `dav_methods` (`PUT`, `DELETE`) делает её доступной на запись по WebDAV
      и требует модификатор `auth_request` в маршруте
...
- Модификаторы (заголовки, правила rewrite)
    ...
    - `auth_request` — авторизовать каждый запрос маршрута подзапросом к
      пулу `pool` по пути `path`; бэкенд получает исходные метод, URI и
      `Authorization` (без тела) и отвечает 2xx, чтобы разрешить, или
      401/403, чтобы запретить. Не больше одного на маршрут
