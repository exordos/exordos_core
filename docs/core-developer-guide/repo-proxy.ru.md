...
Вызывающий с токеном проекта читает репозитории своего проекта и проекта
администратора (`00000000-0000-0000-0000-000000000000`), в котором лежат
общие репозитории реалма; фильтр по любому другому проекту запрещён. Запись
(`update`, `delete`, `refresh`, `upload`) по-прежнему ограничена своим
проектом. Вызывающий без проекта, у которого есть право, читает и пишет всё.

Право `repo.repository.refresh_all` снимает это ограничение только для
`refresh`: его обладатель обновляет и общий репозиторий проекта
администратора — обновление перечитывает инвентарь апстрима и ничего не
пишет. Роль `owner` им обладает. Ни одно право не расширяет `refresh` за
пределы того, что вызывающий читает, поэтому репозиторий чужого проекта
остаётся недоступен.
