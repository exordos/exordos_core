...
- **`upgrade(target)`** — переносит состояние установки с текущего элемента на
  целевой (то же имя, другая версия). На целевой элемент переносятся UUID
  runtime EM-элемента и записи `RepoElementDepsBinding`; он же возвращается
  методом: после обновления установленным считается целевой элемент, поэтому
  `uninstall()` нужно вызывать по его UUID.
