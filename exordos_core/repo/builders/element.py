...
class InstalledManifest(
    ...
    """The model represents an installed element.

    TODO: Write what is the `InstalledManifest`???
    """
    ...
    def get_resource_target_fields(self) -> tp.Collection[str]:
        ...
        """
        return frozenset(("uuid", "name", "version", "manifest", "project_id"))

    @classmethod
    def from_repo_element(cls, element: models.RepoElement) -> "InstalledManifest":
        uuid = element.manifest.get("uuid") or element.uuid
        return cls(
            uuid=uuid,
            name=element.name,
...
class RepoElementBuilderService(
    ...
    def can_update_instance_resource(
        ...
        if self._require_release(instance):
            ...
            pending_resource = ua_models.TargetResource.objects.get_one_or_none(
                filters={
