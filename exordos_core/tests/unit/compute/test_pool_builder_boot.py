...
def test_an_update_keeps_a_machine_that_is_still_installing_on_the_network(builder):
    ...
    machine = _machine()
    boot = _derivatives(
        builder, machine, guest_pair=(_guest(ua_pool.BootAlternative.network), None)
    )
    assert boot == ua_pool.BootAlternative.network.value
...
def test_an_update_leaves_an_installed_machine_on_its_disk(builder):
    ...
    boot = _derivatives(
        ...
        machine,
        guest_pair=(
            _guest(ua_pool.BootAlternative.hd0),
            _guest(ua_pool.BootAlternative.hd0),
        ),
    )
    assert boot == ua_pool.BootAlternative.hd0.value
...
def test_a_changed_image_sends_the_machine_back_to_the_network(builder):
    ...
    boot = _derivatives(
        ...
        guest_pair=(
            _guest(ua_pool.BootAlternative.hd0),
            _guest(ua_pool.BootAlternative.hd0, image="http://repo/other.raw.zst"),
        ),
    )
    assert boot == ua_pool.BootAlternative.network.value
