...
class FakeSession:
    """Stands in for the row the conditional UPDATE competes over."""
    ...
    def execute(self, statement, values):
        ...
        counter, _uuid, guard = values
        if self.stored is None or self.stored < guard:
            self.stored = counter
            return FakeCursor(1)
        return FakeCursor(0)
...
class TestValidateOtp:
    ...
    def test_records_the_consumed_time_step(self, user, secret):
        before = current_timecode(secret)

        user.validate_otp(pyotp.TOTP(secret).now())

        assert user.last_otp_counter in (before, current_timecode(secret))
    ...
    def test_burn_is_a_conditional_update(self, user, secret, session):
        ...
        assert "last_otp_counter IS NULL OR last_otp_counter < %s" in statement
        assert values[0] == values[2] == user.last_otp_counter


class TestActivateOtp:
    def test_burns_the_activation_code(self, secret):
