...
from exordos_core.boot_api.api import app as boot_app
...
from exordos_core.orch_api.api import app as orch_app
from exordos_core.secret import constants as sc
from exordos_core.secret.dm import models as secret_models
from exordos_core.status_api.api import app as status_app
...
def boot_api_service():

    class ApiRestService(test_utils.RestServiceTestCase):
        __FIRST_MIGRATION__ = FIRST_MIGRATION
        __APP__ = boot_app.build_wsgi_application()
...
@pytest.fixture(scope="module")
def status_api_service():

    class ApiRestService(test_utils.RestServiceTestCase):
        __FIRST_MIGRATION__ = FIRST_MIGRATION
        __APP__ = status_app.build_wsgi_application()
...
@pytest.fixture()
def boot_api_noauth_client(boot_api_service):
    return iam_clients.GenesisCoreTestNoAuthRESTClient(
        f"{boot_api_service.get_endpoint()}v1/"
    )


@pytest.fixture()
def orch_api_noauth_client(orch_api_service):
    return iam_clients.GenesisCoreTestNoAuthRESTClient(
        f"{orch_api_service.get_endpoint()}v1/"
    )


@pytest.fixture()
def status_api_noauth_client(status_api_service):
    return iam_clients.GenesisCoreTestNoAuthRESTClient(
        f"{status_api_service.get_endpoint()}v1/"
    )
...
@pytest.fixture
def default_node_uuid():
    return sys_uuid.UUID("00000000-1112-0000-0000-000000000000")
...
def default_node(
    ...
    user_api_client: iam_clients.GenesisCoreTestRESTClient,
    auth_user_admin: iam_clients.GenesisCoreAuth,
    default_node_uuid: sys_uuid.UUID,
):
    default_node = node_factory(uuid=default_node_uuid)
    client = user_api_client(auth_user_admin)
    ...
    url = client.build_resource_uri(["compute", "nodes", default_node_uuid])
    client.delete(url)
