...
try:  # the filter expression language, released or not
    from restalchemy.api import filter_lang  # noqa: F401

    HAS_FILTER_LANG = True
except ImportError:
    HAS_FILTER_LANG = False
...
class TestRecordTags:
    ...
    @pytest.mark.skipif(
        not HAS_FILTER_LANG,
        reason="needs a restalchemy with the filter expression language",
    )
