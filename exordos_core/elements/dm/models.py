...
from exordos_core.common import utils as cm_utils
...
class Manifest(
    ...
    def uninstall(self, session: tp.Any | None = None) -> "Manifest":
        ...
        elements = Element.objects.get_all(
            filters={"manifest": ra_filters.EQ(self.uuid)}, session=session
        )
...
class ElementEngine:
    ...
    def add_resource_by_export(self, export: "Export") -> None:
        ...
        if export.full_link in self._resource_exports:
            raise exceptions.ValidateException(
                err=f"Resource export with full_link '{export.full_link}' already exists."
            )
